...
for an actual Muro deployment, which eases the process of designing frontend modules.
...
As of this writing Muro uses php *7.4.x* and Phalcon *4.1.x*.
...
absolute paths to both the Muro core files and the plugins you wish to expose.
...
Symlink the socket to the expected location (which Muro uses) so both our UI and command
